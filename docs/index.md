...
> AI-powered feature store for data science teams.

featcat is a local-first feature store that helps a data team know **what features exist**, **what they mean**, **where they live**, **whether they're healthy**, and **how to serve their latest values**. It indexes Parquet/CSV sources you point at it, generates documentation via a local LLM, monitors feature drift over time, builds training datasets, tracks materialization runs, and exposes online read/write endpoints through a CLI, REST API, web UI, and Python SDK.
...
- **Reuse** them from notebooks and serving paths via the [SDK](sdk/quickstart.md) — no copy-pasting Parquet paths
...
- You have a fully managed enterprise feature store already (Feast, Tecton, Databricks Feature Store) and only need hosted infrastructure — featcat is a lightweight local-first alternative for teams that want registry, monitoring, dataset builds, and online materialization in one OSS service.
...
| Register your first source | [First Feature](getting-started/first-feature.md) |
