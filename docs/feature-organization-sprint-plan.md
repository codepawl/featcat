...
`BusinessMetric`/`MetricDefinition` là registry object tùy chọn nhưng nên được thiết kế sớm:
...
- Nếu `metric_level` khác `entity_grain`, phải có rollup hoặc aggregation rule rõ ràng.
...
- Nếu `metric_level` khác `entity_grain`, feature phải khai báo `aggregation` hoặc rollup path qua `EntityRelationship`.
...
- Thêm validate BusinessMetric: taxonomy hợp lệ, mapped features tồn tại, metric level khác entity grain phải có aggregation/rollup rule.
...
- Registry lưu được source, entity, relationship, feature view, feature và feature set.
