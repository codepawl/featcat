...
Mỗi GitHub Release sẽ tự động build và push image multi-arch
(`linux/amd64` + `linux/arm64`) lên GHCR:

```bash
docker pull ghcr.io/codepawl/featcat:latest
```

Tag có sẵn: `latest`, `<version>` (vd. `0.2.5`), `<major>.<minor>` (vd. `0.2`).

Để dùng image này thay vì build local, sửa `docker-compose.yml`:

```yaml
  featcat:
    # build: .                                  # bỏ dòng này
    image: ghcr.io/codepawl/featcat:latest      # thêm dòng này
```

## Cài đặt lần đầu (build từ source)
